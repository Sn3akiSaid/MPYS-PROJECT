...
!--------Presets
      ...
      integer,parameter::nkpath=3,np=80,npartitions=30 !Adjust these parameters to obtain better resolution around alphacrit and see points closer to an effectively closed gap
      real*8,parameter::B_x = 0d0, B_y = 0.1d1, B_z = 0d0
      !---------Variable allocation
      integer*4 ik,ipart
     ...
     ! real*8 xk(np**2),yk(nk_HAH),kpath(3,np),kxpath(3,np),kypath(3,np)
      ...
      complex*16,allocatable:: Hamr_topological(:,:,:),work(:),H(:,:),Hm(:,:)
    ...
    integer ib,is
!------------------------------------------------------
    write(hamil_file_trivial,'(a,a,a)') "hamiltonians/", trim(adjustl(prefix)), "_hr_trivial.dat"
    write(hamil_file_topological,'(a,a,a)') "hamiltonians/", trim(adjustl(prefix)), "_hr_topological.dat"
