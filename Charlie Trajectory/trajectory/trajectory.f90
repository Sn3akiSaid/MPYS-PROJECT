...
!Creates a list of 100 points inbetween BZ points LAL
      ...
      kxtemp1=0d0
       do j=1,(np+1)**2
        k=k+1
        kxtemp2=mesh2(1,k)*bvec(:,1)+mesh2(2,k)*bvec(:,2)+mesh2(3,k)*bvec(:,3)
        mesh(:,k) = kxtemp2
        kxtemp1=kxtemp2
       enddo
...
!------export eigenvalus 
         ...
         do i=1,nb
            do k=1,np**2
              write(100,'(3(x,f12.6))') mesh2(1,k),mesh2(2,k),ene(i,k)-ef(ipart)
              if(ipart.eq.3) write(1000,'(3(x,f12.6))') mesh2(1,k),mesh2(2,k),ene(13,k)-ene(12,k)
            enddo
              write(100,*)
              write(100,*)
         enddo
