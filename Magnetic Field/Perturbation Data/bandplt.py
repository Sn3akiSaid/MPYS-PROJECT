...
x1 = data[::50, 0]  # First column (k_y)
y1 = data[::50, 1]  # Second column (E-E_F for first plot)
c1 = data[::50, 2]  # Fourth column (color data for first plot)

x2 = data[:, 0]  # First column (k_y again)
y2 = data[:, 5]  # Sixth column (E-E_F for second plot)
c2 = data[:, 6]  # Eighth column (color data for second plot)

# x1 = data[::50, 0]  # First column (k_x)
# y1 = data[::50, 1]  # Second column (E-E_F for first plot)
# c1 = data[::50, 3]  # Fourth column (color data for first plot)
...
ax.scatter(x1, y1, c=c1, cmap=cmap_name, linestyle='--', s=1)
ax.scatter(x2, y2, c=c2, cmap=cmap_name, s=1)
...
ax.set_xlabel(r'$k_{y}$ (Å$^{-1}$)', fontsize=18)
# ax.set_xlabel(r'$k$ (Å$^{-1}$)', fontsize=18)
# ax2=ax.twiny()
# ax2.set_xlabel('L  < ------------------------   A   ------------------------ >  H')
ax.set_ylabel(r'$E-E_{F}$ (eV)', fontsize=18)
...
plt.tight_layout()
plt.savefig('DataNEWESTky.png', format='png', bbox_inches='tight')
