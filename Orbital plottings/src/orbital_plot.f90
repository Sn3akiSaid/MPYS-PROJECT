...
!--------to be midified by the usere
      ...
      integer,parameter::nkpath=3,np=500
...
!------------------------------------------------------
      ...
      real*8,allocatable:: rvec(:,:),ene(:,:),rwork(:),wt(:,:),wb(:,:),wi(:,:),w(:)
...
111   read(98,'(a)')line
      ...
      read(98,*)bvec
!---------------kpath
      data kpath(:,1) /     third,      third,    0.5d0/  !L
...
!---- Fourrier transform H(R) to H(k)
      ...
      do k=1,nk
         ...
         do j=1,nr
            ...
            do i1=1,nb
               do i2=1,nb
                  Hk(i1,i2)=Hk(i1,i2)+Hamr(i1,i2,j)* &
      ...
      do i=1,nb
         do k=1,nk
           write(100,'(5(x,f12.6))') xk(k),ene(i,k),wb(i,k),wt(i,k),wi(i,k)
         enddo
           write(100,*)
           write(100,*)
      enddo
      call write_plt(nkpath,xkl,klabel,ef)!,kz)
      stop
