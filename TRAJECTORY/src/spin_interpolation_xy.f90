...
!--------Presets
      ...
      integer,parameter::nkpath=3,np=30,npartitions=1 !Adjust these parameters to obtain better resolution around alphacrit and see points closer to an effectively closed gap
!---------Variable allocation
      integer*4 ik,ipart
      ...
      integer*4 i,j,k,n,nr,i1,i2,nb,lwork,info,o,p,j1,j2,steps
     ...
     ! real*8 xk(np**2),yk(nk_HAH),kpath(3,np),kxpath(3,np),kypath(3,np)
      ...
      complex*16,allocatable:: Hamr_topological(:,:,:),work(:)
      	real(8), parameter :: x_min = -0.04d0, x_max = 0.04d0, y_min = -0.04d0, y_max = 0.04d0
            ...
            do k=1,(np+1)**2
                 ...
                 ! enddo
                  write(100,'(12(x,f12.6))') mesh(1:2,k),&
                                             ene(13,k)-minval(ene(13,:)) !This now writes into the files the coordinates as a function of the TCB and BCB energy difference
