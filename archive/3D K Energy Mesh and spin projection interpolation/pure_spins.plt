...
set output sprintf("spins_%s_band_%d.pdf", phase, band)
stats file using 6 nooutput
print STATS_max,STATS_min

set palette defined (STATS_min "blue",0 "white",STATS_max "red")
...
set cbtics (\
    "-0.6" -0.2, \
     "0.6" 0.2) font "Times New Roman,10" offset 0.22,2.2

set cblabel "S_{z}" offset 0.22,4 font "Times New Roman,10"
...
plot \
    file using 1:2:6 with image palette  notitle, \
    file every 31 using 1:2:($4/($7*60)):($5/($7*60)) with vectors head filled size screen 0.03,15,60 lt rgb "black" notitle
