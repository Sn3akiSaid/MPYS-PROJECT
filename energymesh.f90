...
!--------to be midified by the usere
      ...
      integer,parameter::nkpath=3,np=500,npartitions=3 !Adjust these parameters to obtain better resolution around alphacrit and see points closer to an effectively closed gap
!------------------------------------------------------
      integer*4 ik,ipart
...
!Creates a list of 100 points inbetween BZ points LAL
      ! Dont thin you need to do this if u arent using it?
      ! k=0
      ! kxtemp1=0d0
      !  do j=1,(np+1)**2
      !   k=k+1
...
!------export eigenvalus 
         ...
         write(line,'(3a)') 'mesh_closing_200',trim(adjustl(partnumber)),'.dat' 
      ...
      !    do i1=1,nb
            ...
            do k=1,(np+1)**2
                  !write_values(12:13) = 0.0  ! Assuming i ranges from 11 to 14
        
                  !do i = 12,13
                      ! Check if ene(i,k) - ef(ipart) is less than 0.01 and set it to 0 if true
                      if (abs(ene(13,k)-minval(ene(13,:))) .lt. 0.001) then
                          write_values(i) = 0.0
                      else
                          write_values(i) = ene(13,k)-minval(ene(13,:))
                      end if
                  !end do
                  write(100,'(3(x,f12.6))') mesh(1:2,k),&
                                            write_values(i)!,(spin(1:3,i,k),&!need to minimize the energy wrt fermi energy
                                          !sqrt(spin(1,i,k)**2 +spin(2,i,k)**2 +spin(3,i,k)**2), i=12
                                          !This now writes into the files the coordinates as a function of the TCB and BCB energy difference
